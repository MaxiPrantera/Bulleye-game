window.addEventListener('load', function() {
    ...
    class Player{
        ...
        draw(context){
            ...
            if(this.game.debug){
                ...
                context.save(); //creates a snapshot of the current canvas satate, including fillStyle, lineWidth, opacity as well as transformations ans scaling
                context.globalAlpha = 0.5;
                context.fill();
                context.restore();
                context.stroke();
                context.beginPath();
                context.moveTo(this.collisionX, this.collisionY); //define starting x and y coordinates of the line
                context.lineTo(this.game.mouse.x, this.game.mouse.y); //define ending x and y coordinates of the line
        ...
        update(){
            ...
            //sprite animation
            const angle = Math.atan2(this.dy, this.dx); //retorna el angulo en radianes entre el punto (0,0) y el punto (x,y)
            ...
            this.game.obstacles.forEach(obstacle => {
                //[(distance < sumOfRadii), distacne, sumOfRadii, dx, dy];
                let [collision, distance, sumOfRadii, dx, dy] = this.game.checkCollision(this, obstacle); //seria como hacer esto let collision = this.game.checkCollision(this, obstacle)[0];
    ...
    class Egg{
        ...
        update(deltaTime){
            ...
            //collision
            let collisionObject = [this.game.player, ...this.game.obstacles, this.game.enemies]; //spread operator allows us to quickly expand elements in an array into another array
    ...
    class Larva{
        ...
        update(){
            ...
            let collisionObject = [this.game.player, ...this.game.obstacles, ...this.game.eggs]; //spread operator allows us to quickly expand elements in an array into another array
            collisionObject.forEach(object => {
                let [collision, distance, sumOfRadii, dx, dy] = this.game.checkCollision(this, object);
                if(collision){
                    const unit_x = dx / distance;
    ...
    class Enemy{
        ...
        update(){
            ...
            let collisionObject = [this.game.player, ...this.game.obstacles]; //spread operator allows us to quickly expand elements in an array into another array
            collisionObject.forEach(object => {
                let [collision, distance, sumOfRadii, dx, dy] = this.game.checkCollision(this, object);
                if(collision){
                    const unit_x = dx / distance;
    ...
    //aca tengo 1 parent class y 2 child classes
    ...
    class Game{
        ...
        //add eggs periodically
            ...
            //if(this.debug){
            context.fillText('Lost:  ' + this.lostHatchlings, 25, 85);
            //}
            context.restore();
        ...
        checkCollision(a, b){  //para que no se superpongan los obstaculos, true si se superponen, false si no
        ...
        removeGameObjects(){
            this.eggs = this.eggs.filter(object => !object.markedForDeletion); //crea una copia del array y solo contiene los elementos que cumplen la condicion
            this.hatchlings = this.hatchlings.filter(object => !object.markedForDeletion);
            this.paticles = this.particles.filter(object => !object.markedForDeletion);
        }
        ...
        init(){                              //para que no se superpongan los obstaculos
            for (let i = 0; i < 5 ; i++){
                this.addEnemy();
